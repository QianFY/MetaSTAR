...
class SimpleReplayBuffer(ReplayBuffer):
    def __init__(
        ...
        self._pre_rewards = np.zeros((max_replay_buffer_size, 1))
        self._terminals = np.zeros((max_replay_buffer_size, 1), dtype='uint8')
        self.clear()

    def add_sample(self, observation, action, reward, terminal,
                   next_observation, pre_reward, **kwargs):
        self._observations[self._top] = observation
        self._actions[self._top] = action
        self._rewards[self._top] = reward
        self._terminals[self._top] = terminal
        self._next_obs[self._top] = next_observation
        self._pre_rewards[self._top] = pre_reward
        self._advance()
    ...
    def add_samples(self, observations, actions, rewards, terminals,
                   next_observations, pre_rewards, **kwargs):
        ...
        self._pre_rewards[self._top:self._top + n_samples] = pre_rewards
    ...
    def sample_data(self, indices):
        return dict(
            ...
            pre_rewards=self._pre_rewards[indices],
        )
    
    def sample_random_context(self, context_size):
        start = np.random.randint(0, self._size - context_size)
        indices = list(range(start, start + context_size))
