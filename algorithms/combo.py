...
class COMBO(nn.Module):
    def __init__(self,
        ...
        # self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.policy_optim, args.num_iters)
    ...
    def update(self, obs, action, reward, next_obs, done, real_obs, real_action, **kwargs):
        ...
        random_actions = torch.FloatTensor(
            batch_size * self._num_repeat_actions, action.shape[-1]
        ).uniform_(-1.0, 1.0).to(ptu.device)
        tmp_obs = obs.unsqueeze(1) \
            .repeat(1, self._num_repeat_actions, 1) \
            .view(batch_size * self._num_repeat_actions, obs.shape[-1])
        tmp_next_obs = next_obs.unsqueeze(1) \
            .repeat(1, self._num_repeat_actions, 1) \
            .view(batch_size * self._num_repeat_actions, obs.shape[-1])
        
        obs_pi_value1, obs_pi_value2 = self.calc_pi_values(tmp_obs, tmp_obs)
        next_obs_pi_value1, next_obs_pi_value2 = self.calc_pi_values(tmp_next_obs, tmp_obs)
        random_value1, random_value2 = self.calc_random_values(tmp_obs, random_actions)

        obs_pi_value1 = obs_pi_value1.view(batch_size, self._num_repeat_actions, 1)
        obs_pi_value2 = obs_pi_value2.view(batch_size, self._num_repeat_actions, 1)
        next_obs_pi_value1 = next_obs_pi_value1.view(batch_size, self._num_repeat_actions, 1)
        next_obs_pi_value2 = next_obs_pi_value2.view(batch_size, self._num_repeat_actions, 1)
        random_value1 = random_value1.view(batch_size, self._num_repeat_actions, 1)
        random_value2 = random_value2.view(batch_size, self._num_repeat_actions, 1)
        ...
        q1, q2 = self.qf1(real_obs, real_action), self.qf2(real_obs, real_action)
        ...
        qf1_loss = qf1_loss + conservative_loss1
        qf2_loss = qf2_loss + conservative_loss2
    ...
    def calc_pi_values(self, obs_pi, obs_to_pred):
        action, _, _, log_prob = self.act(obs_pi, return_log_prob=True)
        q1 = self.qf1(obs_to_pred, action)
        q2 = self.qf2(obs_to_pred, action)

        return q1-log_prob.detach(), q2-log_prob.detach()
    
    def calc_random_values(self, obs, random_action):
        q1 = self.qf1(obs, random_action)
        q2 = self.qf2(obs, random_action)
        log_prob1 = np.log(0.5**random_action.shape[-1])
        log_prob2 = np.log(0.5**random_action.shape[-1])
    ...
    def update_critic(self, obs, action, reward, next_obs, done, real_obs, real_action, **kwargs):
        # computation of critic loss
        #torch.autograd.set_detect_anomaly(True)
        with torch.no_grad():
            next_action, _, _, next_log_prob = self.act(next_obs, return_log_prob=True)
